...
const CompleteLater = ({ handleChange, handleSave, name }) => {
  return (
    <div className={"flex-1 my-auto"}>
      ...
      <div className={"px-3 border-l border-l-gray-400 my-4 flex-1"}>
        ...
        <p className={"leading-6 text-sm mt-2"}>
          {" "}
          Resume your application by searching for your name under Admissions in
          the homepage
        </p>
        ...
        <div className={"my-2"}>
          ...
          <div className={"flex p-1 border-b border-gray-400"}>
            <input
              type="text"
              className={"py-2 flex-1 bg-inherit focus:outline-none"}
              value={name}
              onChange={(e) => handleChange(e)}
            />
            <button className={"text-sm font-bold py-2.5"} onClick={handleSave}>
              Save
            </button>
          </div>
        </div>
