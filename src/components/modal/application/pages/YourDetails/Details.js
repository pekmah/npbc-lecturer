        ...
        <div className={"flex-[1] flex flex-col"}>
            ...
            <div className={"px-3 border-l border-l-gray-400 my-4 flex-1"}>
                <ul className={"py-3 flex flex-col "}>
                    {details?.map((prog, key) => (

                        <li key={key} className={"py-2 focus:outline-none text-sm font-semibold flex gap-4"}>

                            <input onClick={() => setCurrent(key)} checked={current === key} id="default-radio-1"
                            ...
                            <label htmlFor="default-radio-1"
                                   className={`ml-2 text-sm font-medium ${current === key ? "text-c-blue" : "text-gray-400"} dark:text-gray-300`}>{prog}</label>
