...
function Table({title, headerList, data}) {
    return (
        <div className={"w-full "}>
            {/*Title*/}
            <div className={"py-3 px-1 rounded-sm border-gray-300 border text-c-blue"}>
                <h3 className={"c-xl font-semibold"}>
                    {title}
                </h3>
            </div>
            {/*Table Body*/}
                ...
                <tr className={"bg-bg-blue"}>
                    {
                        headerList?.map((item, key) => (
                            <th key={key} className={`text-left py-2 px-1 w-${headerList?.length + 1}`}>{item}</th>
                        ))
                    }

                    <th className={`text-left py-2 px-1 w-${headerList?.length + 1}`}>Actions</th>
                        ...
                        <tr key={key} className={"border border-line-gray font-medium"}>
                            {
                                headerList.map((cKey, key) => (
                                    <td key={key}
                                        className={`text-sm py-3 px-1`}>{item[cKey?.toLowerCase()]}
                                    </td>
                                ))
                            }
                            {/*    Actions column    */}
