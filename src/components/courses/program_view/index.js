    ...
    <div className={"p-3 px-3 md:px-6"}>
      <div className={"flex flex-col md:flex-row mb-4 gap-y-2"}>
        <div className={"md:flex-[0.4]"}>
          <h4 className={"text-xl text-c-blue font-semibold"}>
            Program Brief View
          </h4>
        </div>

        <div
          className={"font-medium md:font-semibold text-sm text-c-blue flex-1"}
        >
          This table shows all our programs with brief details for a unified
          view that makes comparison easy and convenient. All extra details on
          every course are available and can be accessed by choosing course from
          the table or the above groupings
        </div>
      </div>

      <div className={"max-w-[100vw] overflow-x-scroll md:overflow-hidden"}>
        {/*Table*/}
        <Table headerList={table_columns} data={table_data} />
      </div>
    </div>
  );
}
...
const table_data = [
  ...
  },
  {
    ...
    hours: "240",
    criteria:
      "KCSE C- (Minus) or KCE division II, KACE one (1) Principal or an equivalent qualification or Certificate of Experiential Learning or KNQF 5",
    duration: "2 years",
    fees: "ksh 1500",
  },
  {
    ...
    target:
      "Christian Ministers and Workers in the Church and Para Church Orgamisations and Ministries",
    hours: "240",
    criteria: "Open",
    duration: "2 years",
    fees: "ksh 2000",
